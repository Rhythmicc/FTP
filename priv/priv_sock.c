...
void priv_sock_set_nobody_context(session_t sess) {
    close(sess->peer_fd); /// 进制数据端口向控制端口写数据
...
char priv_sock_recv_cmd(int fd) {
    ...
    // 子进程关闭
    if (ret == 0) {                                       // 客户端退出
        printf("Control pipe close!\n");
        exit(EXIT_SUCCESS);
    }
    if (ret != sizeof(res)) {                             // 意外的数据
...
void priv_sock_send_result(int fd, char res) {            // cd模块间发送数据的结果
...
char priv_sock_recv_result(int fd) {                      // cd模块间接受数据
    char res;
    int ret = readn(fd, &res, sizeof res);
    if (ret != sizeof(res)) {
