...
static int is_port_active(session_t sess); /// 判断主动模式是否开启
static int is_pasv_active(session_t sess); /// 判断被动模式是否开启
...
void download_file(session_t sess) {
    ...
    /// 判断是否是普通文件
    struct stat sbuf;
    if (fstat(fd, &sbuf) == -1) ERR_EXIT("fstat");
    ...
    if (lseek(fd, offset, SEEK_SET) == -1) ERR_EXIT("lseek");

    /// 150 ascii
    /// 150 Opening ASCII mode data connection for common.h (996 bytes).
    char text[1024] = {0};
    if (sess->ascii_mode == 1) snprintf(text, sizeof(text), "Opening ASCII mode data connection for %s (%lu bytes).", sess->args, filesize);
    else snprintf(text, sizeof(text), "Opening Binary mode data connection for %s (%lu bytes).", sess->args, filesize);
    ...
    while (nleft > 0) {
...
#else
        int nwrite = sendfile(sess->data_fd, fd, 0, (off_t*)&block_size, NULL, 0);
#endif
        if (sess->is_receive_abor == 1) {
            flag = 2; /// ABOR
            /// 426 Interupt downloading file.
        ...
        nleft -= nwrite;
        limit_cur_rate(sess, nwrite, 0);/// 限速
    }
    if (nleft == 0) flag = 0; /// 正确退出

    /// 清理 关闭fd 文件解锁
    if (unlock_file(fd) == -1) ERR_EXIT("unlock_file");
    close(fd);
    close(sess->data_fd);
    sess->data_fd = -1;

    /// 226
    if (flag == 0) ftp_reply(sess, FTP_TRANSFEROK, "Transfer complete.");
    else if (flag == 1) ftp_reply(sess, FTP_BADSENDFILE, "Sendfile failed.");
    else if (flag == 2) ftp_reply(sess, FTP_ABOROK, "ABOR successful.");
...
void upload_file(session_t sess, int is_appe) {
    ...
    {
        ftruncate(fd, 0); /// 创建新的文件, 如果源文件存在则直接覆盖
    } else if (!is_appe) /// REST + STOR
    {
        /// lseek进行偏移
        ftruncate(fd, offset);
        if (lseek(fd, offset, SEEK_SET) == -1) ERR_EXIT("lseek");
    ...
    {
        if (lseek(fd, 0, SEEK_END) == -1) ERR_EXIT("lseek");
    }
    ...
    while (1) {
        ...
        limit_cur_rate(sess, nread, 1); /// 实行限速
    }
    ...
    /// 226
    if (flag == 0) ftp_reply(sess, FTP_TRANSFEROK, "Transfer complete.");
    else if (flag == 1) ftp_reply(sess, FTP_BADSENDNET, "Reading from Network Failed.");
    else if (flag == 2) ftp_reply(sess, FTP_BADSENDFILE, "Writing to File Failed.");
    else ftp_reply(sess, FTP_ABOROK, "ABOR successful.");
...
void trans_list(session_t sess, int list) {
    /// 发起数据连接
    if (get_trans_data_fd(sess) == 0) return;

    /// 给出150 Here comes the directory listing.
    ftp_reply(sess, FTP_DATACONN, "Here comes the directory listing.");

    /// 传输目录列表
    if (list == 1) trans_list_common(sess, 1);
    else trans_list_common(sess, 0);
...
static int get_trans_data_fd(session_t sess) {
    ...
    /// 主动模式
    if (is_port) get_port_data_fd(sess);

    if (is_pasv) get_pasv_data_fd(sess);

    setup_signal_alarm_data_fd(); /// 安装信号
    start_signal_alarm_data_fd(); /// 开始计时
    return 1;
}
...
static void trans_list_common(session_t sess, int list) {
    ...
    while ((dr = readdir(dir))) {
        ...
        char buf[1024] = {0};
        struct stat sbuf;
        if (lstat(filename, &sbuf) == -1) ERR_EXIT("lstat");
